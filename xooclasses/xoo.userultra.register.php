...
ob_end_clean();
error_log($contents, 3, "/tmp/php-error.log");		
ob_start();
var_dump($value); 
$contents = ob_get_contents();
ob_end_clean();
error_log($contents, 3, "/tmp/php-error.log"); 		
					//	error_log( print_r($key), 3, "/tmp/php-error.log");
 					//	error_log( print_r($value), 3, "/tmp/php-error.log");	 
...
$package = $xoouserultra->paypal->get_package_by_role($user->roles[0]);
