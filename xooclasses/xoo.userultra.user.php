...
class XooUserUser {
    ...
    ******************************************/
    ...
    function update_me()
    {
        ...
        $prof_category_field = "";
        $profcategory_field_name = 'prof_category';
        $profcategory_field_length = strlen($profcategory_field_name);
        ...
        foreach ($array as $key => $field)
        {
            ...
            $stripped_key = str_split($array[$key]['meta'], $profcategory_field_length);
            if ($profcategory_field_name != $array[$key]['meta'] && $stripped_key[0] === $profcategory_field_name) {
                $profcategory_to_name[$array[$key]['meta']] = $array[$key]['name'];
            }
...
//        var_dump($profcategory_to_name);
        ...
        if (!isset($this->errors))
        {
            ...
            foreach($this->usermeta as $key => $value)
            {
                ...
                if ($stripped_key[0] === $profcategory_field_name && $key != $profcategory_field_name && is_array($value))
                {
                    array_push($profcategory_meta_keys, $key);
                //    $value = implode(', ', $value);
...
//                var_dump($value);
                echo "<br/>111<br/>";
                array_push($profcategory_meta_names, $profcategory_to_name[$value]);
            }
            $profcategory_meta_names_str = implode(', ', $profcategory_meta_names);
            echo "<br/>***<br/>";
            echo("profcategory_meta_names_str: $profcategory_meta_names_str");
            echo "<br/>***<br/>";
...
//echo "<br/>";
                    ...
                    $id = "uultra-search-input-" . $value['name'];
                    if ($counter > 0 && $counter % 2 == 0)
 ...
 //                       if () {
 //                           wp_enqueue_script('populateCities', WP_CONTENT_URL . '/themes/i-max/js/regions.js');
 ...
 //                       }
            ...
            if (isset($this->checkbox_search_fields) && count($this->checkbox_search_fields) > 0) {
                ...
                //        var_dump($value['meta']);
                        $class = 'uultra-search-input uultra-search-meta-' . $value['meta'];
                        $loop = array();
...
//echo "<br/>TEST<br/>";
//$test_var = $xoouserultra->commmonmethods->get_predifined( 'regions' );

//        regions_sities_data['cities'] = ;
//var_dump($regions_sities_data);
//        wp_register_script('populateCities', WP_CONTENT_URL . '/themes/i-max/js/regions.js');
...
//            wp_localize_script( 'populateCities', 'populate_sities_vars', $regions_sities_data);
 //       };
        ...
        //add_action( 'wp_head', 'wp_test_en');
       // $display.='<script language="javascript">populateCities("uultra-search-input-Region", "uultra-search-input-City");</script>';
        $display.='<script language="javascript">populateCities();</script>';
        $display.='</form>';
